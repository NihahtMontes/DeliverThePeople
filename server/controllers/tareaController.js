...
const SELECT_TAREAS = `
  SELECT t.id, t.sucursal_id, t.asignado_a, t.titulo, t.estado, t.completada_en,
...
async function listarTareas(req, res, next) {
  try {
    ...
    const result = await pool.query(
      `${SELECT_TAREAS} ${where}
       ORDER BY CASE t.estado WHEN 'pendiente' THEN 1 WHEN 'en_progreso' THEN 2 WHEN 'completada' THEN 3 ELSE 4 END,
                t.updated_at DESC`,
      valores
    );
...
async function crearTarea(req, res, next) {
  try {
    ...
    const insert = await pool.query(
      `INSERT INTO tareas (sucursal_id, asignado_a, titulo, estado, created_at)
...
async function actualizarEstado(req, res, next) {
  try {
    ...
    if (!ESTADOS.includes(estado)) return res.status(400).json({ ok: false, error: 'Estado no valido.' });
    if (!esGestor(req) && !['en_progreso', 'completada'].includes(estado)) {
      return res.status(403).json({ ok: false, error: 'Solo puedes iniciar o completar tus tareas.' });
    }

    const valores = [id];
    let sql = 'SELECT id, asignado_a, sucursal_id FROM tareas WHERE id = $1';
    if (!esGestor(req)) {
      valores.push(req.user.id);
      sql += ` AND asignado_a = $${valores.length}`;
    } else if (gerenteConSucursal(req)) {
      valores.push(req.user.sucursal_id);
    ...
    await pool.query('UPDATE tareas SET estado = $1, updated_at = now() WHERE id = $2', [estado, id]);
